<template>
  <base-dialog-form
    ...
    :max-width="800"
    :value="dialogChangeDeviceInfoCertificateState"
    :disableSubmitButton="!certificateList || certificateList.length === 0"
    :sending="sending"
    :loading="loading"
  >
    <template slot="dialogTitle">
      {{ $t('page.certificate_deviceInfo.dialog_changeCertificate.title') }}
    </template>
                    ...
                    <tbody>
                      <tr>
                        <td class="pa-2 grey--text text--darken-1" width="25%">
                          {{
                            $t(
                              'page.certificate_deviceInfo.dialog_changeCertificate.th_createDate'
                            )
                          }}
                        </td>
                      ...
                      <tr>
                        <td class="pa-2 grey--text text--darken-1" width="25%">
                          {{
                            $t(
                              'page.certificate_deviceInfo.dialog_changeCertificate.th_uploadDate'
                            )
                          }}
                        </td>
                        <td class="pa-2 word-break">
                      ...
                      <tr>
                        <td class="pa-2 grey--text text--darken-1" width="25%">
                          {{
                            $t(
                              'page.certificate_deviceInfo.dialog_changeCertificate.th_expiryDate'
                            )
                          }}
                        </td>
                        <td class="pa-2 word-break">
...
import {
  apiChangeDeviceInfoCertificate,
  apiGetDeviceInfoCertificateList
} from '@/api/apiClient/certificate_deviceInfoCertificate';
...
export default {
  ...
  watch: {
    dialogChangeDeviceInfoCertificateState(val) {
      if (val) {
        this.getDeviceInfoCertificateList();
      }
    }
  },

  methods: {
    async getDeviceInfoCertificateList() {
      this.loading = true;
      this.errorMessage = null;
      try {
        let response = await apiGetDeviceInfoCertificateList();
    ...
    async submitForm() {
      this.sending = true;
      try {
        await apiChangeDeviceInfoCertificate(this.selectedItem);
        this.errorMessage = null;
    ...
    closeDialog() {
      this.errorMessage = null;
      this.$emit('update:dialogChangeDeviceInfoCertificateState', false);
    }
  }
};
</script>
